...
print_help() {
cat << HEREDOC
Test the production server initialisation
...
echo "Testing site $sitename_var on a container"
...
# todo create container. plain ubuntu 20 server


sudo apt install git

# 1) set up ssh keys   #todo James: not sure how this works with the container. the user has access to the container so probably not needed for the test.
# todo James: But the other scripts rely upon ssh, so I think it will still need to be set up.
# todo James: Is the container site locally visible as an IP? Maybe the container site can be referred to as con_$sitename_var, eg con_varc.
# todo James: current a site such as varc, has prod_varc and stg_varc so setting up con_varc would work. It would not be too hard to setup.
# todo James: There is a script here: https://deliciousbrains.com/ssl-certificate-authority-for-local-https-development/ to create our own ssl cert for local site.
# todo James: The secrets file could then be auto created for the container site.

  #2) pl updateserver [site] # this pushes the scripts up.
  pl updateserver [site] # todo Rob: I will update this script later (during sprint?) to get this to work.
    #3) change example.secrets.sh to secrets.sh and edit it.
  # todo Rob: I'll create some code that will make a temporary secrets.sh file, put in the correct info for the con version of the site and push it to container.

  #4) run initserver.sh #todo James: this should just work now.

  #5) run prodow (which calls createsites.sh) #todo Rob: I'll need to work on this so it will push to the container.
  # todo Rob: I'll make sure it pushes the stg version to the container (which has a local production version).
  # todo Rob: I'll then push the dev changes to the container
  #todo James: Now the tests can be run on the container since it would have the test site.


# todo James: all the code below maybe useful or not I haven't checked it. This is all a bit rough, and we can work together to improve it.
...
pl backup $sitename_var "pretest"
